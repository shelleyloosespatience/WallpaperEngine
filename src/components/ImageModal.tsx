...
const ImageModal = ({ image, onClose, onSetWallpaper, isLoading }: ImageModalProps) => {
  ...
  const [url4k, setUrl4k] = useState<string | null>(null); // 4K download URL for MotionBGs
  const [isResolving, setIsResolving] = useState(false);
  const [isSettingVideo, setIsSettingVideo] = useState(false); // Loading state for video wallpaper setting
    ...
    if (image.source === 'wallpaperflare' && image.detailUrl) {
      ...
      (async () => {
        try {
          ...
          if (result?.success && result?.url && !hasResolvedRef.current) {
            hasResolvedRef.current = true;
            // Preview URL for quick display
            setHighResUrl(result.url);
            setDisplayUrl(result.url); // Show preview video in modal
            // 4K download URL for actual wallpaper setting
        ...
        <div className="p-4 border-t border-gray-900/50 space-y-2">
          ...
          {image.type === 'video' && image.source === 'motionbgs' ? (
            <button
              onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                e.stopPropagation();
                // Use 4K URL if available, otherwise fall back to preview
